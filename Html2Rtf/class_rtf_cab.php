...
class rtf {
	...
	function parseDocument() {
		...
		$doc_buffer = preg_replace("/<DIV align=justify>(.*?)<\/DIV>/mi", "\\qj \\1\\qj0", $doc_buffer);
		$doc_buffer = preg_replace("/<DIV align=right>(.*?)<\/DIV>/mi", "\\qr \\1\\qr0", $doc_buffer);
		$doc_buffer = preg_replace("/<DIV align=\"right\">(.*?)<\/DIV>/mi", "\\qr\\1\\qr0", $doc_buffer);
		$doc_buffer = preg_replace("/<DIV align=left>(.*?)<\/DIV>/mi", "\\ql \\1\\ql0", $doc_buffer);
		$doc_buffer = preg_replace("/<DIV align=center>(.*?)<\/DIV>/mi", "\\qc \\1\\qc0", $doc_buffer);
		
		$doc_buffer = preg_replace("/<div align=\"right\">(.*?)<\/div>/mi", "\\qr\\1\\qr0", $doc_buffer);
		$doc_buffer = preg_replace("/<FONT face=\"arial\">(.*?)<\/FONT>/mi", "\\f0 \\1", $doc_buffer);
	  ...
	  //$doc_buffer = preg_replace("/<H2>(.*?)<\/H2>/mi", "\\pard\\qc\\fs32 \\1\\par\\pard\\fs{$this->font_size} ", $doc_buffer);
		...
		$doc_buffer = preg_replace("/<span style=font-size: 13px;>(.*?)<\/span>/mi", "\\fs26 \\1\\fs{$this->font_size} ", $doc_buffer);
