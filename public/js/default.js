...
function fc_ajax_comp(tabela, campo0, input0, unir, id_ref, id_val, conex) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (unir == 'unir') {
                for (b in iSinput) {
                    var targetId = iSinput[b];
                    if (!targetId) {
                        if (window.console && console.warn) {
                            console.warn('fc_ajax_comp: id de destino vazio', {
                                tabela: tabela,
                                campo0: campo0,
                                input0: input0,
                                unir: unir,
                    ...
                    if (!$target.length) {
                        if (window.console && console.warn) {
                            console.warn('fc_ajax_comp: campo de destino nao encontrado no DOM', {
                                targetId: targetId,
                                tabela: tabela,
                                campo0: campo0,
                                input0: input0,
                    ...
                    $target.val(iSunir || "");
                    //$target.val(($target.val() || "").replace(", , undefined, ",""));
...
function fc_edit_usu(valor1, valor2) {
    ...
    if (valor2 == "I") {
        if (!window.__bancoTemplate) {
            window.__bancoTemplate = $("#banco_0").html();
        }
        $("#banco_0").html(window.__bancoTemplate);
        $("#banco_1").html("");
        $("#banco_num").val(1);
        $("#id_usu").val("");
...
function cpfcnpj_mask(valor) {
    setTimeout(function () {
        let $campo = $("#campo" + valor);
        let valorId = $campo.val().replace(/\D/g, "");
        let valorMask = "";

        if (valorId.length <= 11) {
            // CPF: 000.000.000-00
            valorMask = valorId
                .replace(/^(\d{3})(\d)/, "$1.$2")
...
function merge_fields(valIDs, setID) {
    let partes = valIDs.split("|");
    let resultado = [];
    ...
    partes.forEach(function (item) {
        ...
        let textoLimpo = valor.replace(/^['"]|['"]$/g, "");

        if (/^\d+$/.test(textoLimpo)) {
            let $campo = $("#campo" + textoLimpo);

            if ($campo.length) {
                let conteudo = $.trim($campo.val());

                if (conteudo) {
                    let numeroLimpo = conteudo.replace(/\D/g, "");

                    if (numeroLimpo.length === 11) {
                        resultado.push(", inscrito(a) no CPF sob o nº " + conteudo);
                    } else if (numeroLimpo.length === 14) {
                        resultado.push(", inscrito(a) no CNPJ sob o nº " + conteudo);
                    } else {
...
function mucampos(valor1, valor2) {
    ...
    $("#" + myid_sel + " option").each(function () {
        ...
        if (valor2 == 1) {
            ...
            myCampo = myCampo.toLowerCase().replace(/(?:^|\s)\S/g, function (a) {
...
function sel_tipo(valor1, valor2, selected) {
