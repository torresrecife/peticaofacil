...
/** @var array $qtd */
/** @var int $limit */
/** @var string $usu_nivel */
/** @var string|int $tipoId */
?>
<?php if (count($rows) > 0) { ?>
	<div style="float:left; padding-bottom:5px; margin-left:13px"><label style="padding-top:3px; float:left;">Pesquisa:&nbsp;&nbsp;</label><input type="text" id="search_<?php echo $tipoId; ?>" value="" style="float:left; height:19px; width:260px"/><input type="button" onclick="ajax_pecas_search('<?php echo $tipoId; ?>')" value="Procurar" style="float:left; margin-left:5px; height:23px; cursor:pointer"/></div>
		...
		<tr>
			<td colspan="5" class="order" style="text-align:center">
				<?php 
					...
					echo "<input type='button' onclick='ajax_pecas(".$tipoId.",".($limit-1).",\"\")' style='border:0;background:none;cursor:pointer;$stl2' value='<<'>";
					for ($nr=1; $nr<=$npag; $nr++){
						if($nr>$tpg1 && $nr<$tpg2){
							if($limit==($nr-1)){ $stl = "font-weight:bold;font-size:8pt";}else{ $stl = ""; }
							echo "<input type='button' onclick='ajax_pecas(".$tipoId.",".($nr-1).",\"\")' style='border:0;background:none;cursor:pointer;$stl' value='".$nr."'>";
							$ult = ($nr-1);
						}
					}
					
					if($limit==($npag-1) || $npag<$lipg){$stl3="display:none";}else{$stl3="";}
					echo "<input type='button' onclick='ajax_pecas(".$tipoId.",".($limit+1).",\"\")' style='border:0;background:none;cursor:pointer;$stl3' value='>>'>";
				?>
			</td>
		</tr>
	</table>
